...
class IconSetTableSeeder extends Seeder
{
    /**
    ...
    public function run()
    {
        $icon_map = $this->font_awesome_json();

        foreach ($icon_map as $icon)
        {
            $icon_name = explode('fa-', $icon);

            $icon = IconsSet::firstOrCreate([
                'icon' => $icon,
                'source' => 'font-awesome'
            ]);

            if(is_null($icon->name))
            {
                $icon->name = $icon_name[1];
                $icon->save();
            }
        }
    }

    public function font_awesome_json()
    {
        $icons = [
            "fas fa-address-book",
            "fas fa-address-card",
